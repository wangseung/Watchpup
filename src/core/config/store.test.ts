...
describe('ConfigStore', () => {
  it('applies defaults and persists updates', () => {
    ...
    expect(s1.get().bubbleStackCount).toBe(3)
    expect(s1.get().bubbleDurationSeconds).toBe(10)
    ...
    expect(s1.get().slackNewsEnabled).toBe(false)
    expect(s1.get().slackNewsChannels).toEqual(['all_전사공유', 'all_전사공지', 'all_random'])
    expect(s1.get().reminderListSelectionExplicit).toBe(false)
    s1.update({ mySlackUserId: 'U123', model: 'sonnet', petSizePercent: 150, bubbleSizePercent: 80, bubbleStackCount: 4, bubbleDurationSeconds: 15, hudSizePercent: 70, hudAlignment: 'left', showActivityHud: false, naggingEnabled: true, naggingMinMinutes: 3, naggingMaxMinutes: 9, slackNewsEnabled: true, slackNewsChannels: ['all_random'], slackNewsKeywords: ['iOS'] })
    ...
    expect(s2.get().bubbleStackCount).toBe(4)
    expect(s2.get().bubbleDurationSeconds).toBe(15)
    ...
    expect(s2.get().slackNewsEnabled).toBe(true)
    expect(s2.get().slackNewsChannels).toEqual(['all_random'])
    expect(s2.get().slackNewsKeywords).toEqual(['iOS'])
  })
})
