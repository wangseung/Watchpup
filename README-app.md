...
앱을 처음 실행했을 때 토큰이 없으면 터미널/콘솔에 경고만 남기고 Slack 연동 없이 창은 뜬다. `npm run setup` 실행 후 앱을 재시작하면 연동이 시작된다.
...
같은 값들은 프로젝트 루트의 `watchpup.config.yaml`(없으면 최초 저장 시 생성)로도 직접 편집 가능하다.
