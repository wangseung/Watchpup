...
describe('ReminderGateway', () => {
  ...
  it('작업 생성, 편집, 완료 변경과 링크 추가를 native helper 명령으로 전달한다', async () => {
    ...
    expect(runner).toHaveBeenNthCalledWith(2, 'update-title', ['R2', '변경 제목'])
    expect(runner).toHaveBeenNthCalledWith(3, 'update-user-note', ['R2', '사용자 메모'])
    expect(runner).toHaveBeenNthCalledWith(4, 'set-completed', ['R1', 'true'])
    expect(runner).toHaveBeenNthCalledWith(5, 'append-link', ['R1', 'Jira', 'https://example.atlassian.net/browse/APP-1'])
  })
